...
void stripedCollective(CSR csrMatrix, double** vector, double* result, int rank, int size, int vectorCols) {
    int rowsPerStripe = csrMatrix.numRows / size;
    int startRow = rank * rowsPerStripe;
    int endRow = (rank == size - 1) ? csrMatrix.numRows : startRow + rowsPerStripe;

    // 初始化局部结果数组
    for (int i = 0; i < rowsPerStripe * vectorCols; i++) {
        result[i] = 0.0;
    }
...
int main(int argc, char* argv[]) {
    ...
    stripedCollective(csrMatrix, vector, localResult, startRow, endRow, vectorCols);
    double* globalResult = NULL;
    if (rank == 0) {
        globalResult = (double*)malloc(numRows * vectorCols * sizeof(double));
    }

    MPI_Gather(localResult, rowsPerProc * vectorCols, MPI_DOUBLE, globalResult, rowsPerProc * vectorCols, MPI_DOUBLE, 0, MPI_COMM_WORLD);
